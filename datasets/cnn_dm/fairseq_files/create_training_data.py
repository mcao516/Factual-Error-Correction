...
def create_training_data(data, file_path, sep_token='</s>'):
    ...
    Arguments:
        ...
        file_path {str} -- path to write file.

    Keyword Arguments:
        sep_token {str} -- (default: {'</s>'})
    """
    with open(file_path + '.source', 'w', encoding='utf-8') as sfout, \
         open(file_path + '.target', 'w', encoding='utf-8') as tfout:
        for d in tqdm(data):
